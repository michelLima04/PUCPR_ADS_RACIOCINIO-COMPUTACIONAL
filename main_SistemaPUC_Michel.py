...
list_students = [] # Lista que armazenará Alunos
list_teachers = [] # Lista que armazenará Professores
list_courses = [] # Lista que armazenará Disciplinas
list_classes = [] # Lista que armazenará Turmas
list_id_numbers = [] # Lista que armazenará Matrículas
...
def create_operation(section,operation,list_students): # Função para criar/adicionar dados.
    print(f"\n-----[{section} -> {operation}]-----")
    while True:
    ...
    dictionary_students = {"CÓDIGO": cod, "NOME": name, "CPF": cpf}
    list_students.append(dictionary_students)
    print("----------------------------------------------------------------")
    input(f"> {section} cadastrado com sucesso! Pressione Enter para voltar...")

def read_operation(section,operation,list_students): # Função para ler/listar dados.
    print(f"\n-----[{section} -> {operation}]-----")
...
def update_operation(section,operation,list_students): # Função para atualizar dados.
    print(f"\n-----[{section} -> {operation}]-----")
    if list_students == []:  # Se a lista de Alunos estiver vazia, não tem como atualizar dados...
        ...
        while True:
            ...
            if student_by_edit == None:
                ...
                while True:
                    try:  # Validar que o COD seja um número inteiro maior que zero.
                ...
                while True:  # Validar que o input seja ums String
                ...
                while True:  # Validar que o input seja de 11 digitos
...
def delete_operation(section,operation,list_students): # Função para remover/apagar dados.
    print(f"\n-----[{section} -> {operation}]-----")
...
while loop_menu == True: # Looping da Tela Inicial
    while True:
        try:
    ...
    while loop_operations == True: # Looping da tela secundária de Operações
        ...
        if option == 1:
            operation = "CADASTRAR"
            #copy_list = list_students # Possível implementação futura sendo estudada...
            create_operation(section,operation,list_students)

        elif option == 2:
            operation = "LISTAR"
            # copy_list = list_students # Possível implementação futura sendo estudada...
            read_operation(section,operation,list_students)

        elif option == 3:
            operation = "ATUALIZAR"
            # copy_list = list_students # Possível implementação futura sendo estudada...
            update_operation(section, operation, list_students)

        elif option == 4:
            operation = "EXCLUIR"
            # copy_list = list_students # Possível implementação futura sendo estudada...
            delete_operation(section, operation, list_students)
